...
export default function Galeria() {
    ...
    useEffect(() => {
        const getMediaUrls = async () => {
            ...
            let subdirectories = ['mago', 'palhaco', 'animador']; // array of subdirectories
            let imageUrls = [];
            let videoUrls = [];
    
            // Iterate over each directory and get the URLs
            for (let directory of subdirectories) {
                // Get the references
                let imgListRef = ref(storage, `imagens/${directory}`);
                let videoListRef = ref(storage, `videos/${directory}`);
    
                // Get URLS and array-spreading them into a single array
                let imgData = await getUrls(imgListRef);
                let vidData = await getUrls(videoListRef);
    
                imageUrls = [...imageUrls, ...imgData];
                videoUrls = [...videoUrls, ...vidData];
            }
    ...
    return (
        <>
            <Head>
                <title>Tio Batata: Galeria</title>
            </Head>
            <NavMinima toggleContent={toggleContent} titulo={"GALERIA"} />
