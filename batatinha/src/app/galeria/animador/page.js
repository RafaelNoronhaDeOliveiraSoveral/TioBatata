...
export default function Animador() {
    ...
    useEffect(() => {
        const getMediaUrls = async () => {
            // Get the references
            let imgListRef = ref(storage, 'imagens/animador');
            let videoListRef = ref(storage, 'videos/animador'); // Assuming 'videos' is the directory for videos
    ...
    return (
        <>
            <Head>
                <title>Tio Batata: Animador</title>
            </Head>
            <NavMinima toggleContent={toggleContent} titulo={"ANIMADOR"} />
            ...
            {displayedContent === 'photos' && (
                ...
                (
                    <h1 className="text-gray-500 text-2xl m-4">{conteudo}</h1>
                )
            )}
